...
class Follower:
	...
	def image_callback(self, msg):
		...
		mask_yellow = cv.inRange(hsv, lower_yellow1, upper_yellow1)	#閾値によるHSV画像の2値化（マスク画像生成） 
		...
		if M_yellow['m00'] > 0:
			...
			self.cmd_vel_pub.publish(self.twist)
			cmd == 2
			cv.putText(image, 'Thank you!!', (200, 266), cv.FONT_HERSHEY_PLAIN, 4, (0, 0, 255), 5, cv.LINE_AA)
